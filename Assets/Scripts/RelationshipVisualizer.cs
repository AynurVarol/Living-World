...
public class RelationshipVisualizer : MonoBehaviour
{
    ...
    private Dictionary<int, LineRenderer> lines = new Dictionary<int, LineRenderer>();//her npc için cizgiyi sakla bir kere oluþtur hep kullan performans için iyi object pooling mantýgý
    ...
    private void Update()
    {
        ...
        foreach(var target in allNPCs) //tüm npclere bak
        {
            if (target == npc) continue; //kendini atla

            float trust = 0; //iliþki yoksa 0 kabul 
     ...
     {
        if(!lines.ContainsKey(target.Id)) //yoksa oluþtur
        {
    ...
    void HighlightConnection(NPC source, NPC target) //etkileþim olunca cizgiyi vurgula
    {
     ...
     IEnumerator FlashLine(LineRenderer line) //çizgi kalýnlasýp ýnceliyor
    {
        float timer = 0f;
        ...
        while(timer < 0.3f)
        {
            ...
            float t = Mathf.PingPong(timer * 10f, 1f); //ileri geri animasyon juice/ feedback
